...
static int do_error(const char * msg, int eno)  
{  
...
{  
	...
	return (semid == -1)? do_error("create sem(semget)", errno):semid;  
}  
...
{  
	...
	return (ret == -1)? do_error("Set Value(semctl)", errno):ret;  
}  

int get_sem(int semid, int index)  
{  
	int ret = semctl(semid,index,GETVAL,0);  
	return (ret == -1)? do_error("Get Value(semctl)", errno):ret;  
}   
...
{  
	...
	return (ret == -1)? do_error("Wait(semop)", errno):ret;  
}   
...
{  
	...
	int ret;  
	ret= semop (semid, &operation, 1);   
	return (ret == -1)? do_error("Post(semctl)", errno):ret;  
}   
